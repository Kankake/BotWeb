...
bot.start(async ctx => {
  ...
  // 1) отправляем вашу картинку
  await ctx.replyWithPhoto({ source: WELCOME_PHOTO });
  // 2) текст с обращением и клавиатуру «Да»/«Нет»
  await ctx.reply(
    ...
    Markup.keyboard([['Да 👍', 'Нет, ввести другое имя 🗣️']])
      .resize()
      .oneTime()
  );
});
...
bot.hears('Да 👍', async ctx => {
  ...
  // 3) следующая картинка
  await ctx.replyWithPhoto({ source: NEXT_PHOTO });
  // 4) основной выбор способа записи
  await ctx.reply(
    `Очень приятно познакомиться, ${name}!\n` +
    `Предлагаю выбрать вариант для записи:`,
    Markup.keyboard([
...
// Пользователь хочет другое имя
bot.hears('Нет, ввести другое имя 🗣️', async ctx => {
  await ctx.reply('Пожалуйста, введите, как к вам обращаться:');
  // ждём единственный следующий текст
  bot.once('text', async ctx2 => {
    const custom = ctx2.message.text;
    await ctx2.replyWithPhoto({ source: NEXT_PHOTO });
    await ctx2.reply(
      `Очень приятно познакомиться, ${custom}!\n` +
      `Предлагаю выбрать вариант для записи:`,
      Markup.keyboard([
        ['🖥️ Запись онлайн', '📞 Запись по звонку администратора']
      ])
...
bot.on('contact', async ctx => {
  ...
  await ctx.reply('Спасибо! Запись подтверждена, будем очень ждать вас на занятии.', Markup.removeKeyboard());
});

bot.hears('🖥️ Запись онлайн', ctx => {
  ctx.reply(
    'Откройте наше мини-приложение для записи:',
